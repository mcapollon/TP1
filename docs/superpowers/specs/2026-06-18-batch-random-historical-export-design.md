...
**1. `backend/assets/us_common_stocks.txt` (new, committed)**
Plain newline-delimited list of filtered broad US common-stock tickers (~5–7k). Generated by the builder below and checked in so the endpoint reads a static file — no network call at request time. Lives under `backend/assets/` (not `backend/data/`, which is gitignored) so it actually gets committed — the reproducibility anchor must be in version control.
...
Writes the deduplicated, sorted result to `backend/assets/us_common_stocks.txt`. Single responsibility: produce the universe file. Re-runnable to refresh the list later.
